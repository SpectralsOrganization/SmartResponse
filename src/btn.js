 ...
 * – Liste, ajout, suppression (×) et variables $&nom (ou $&amp;nom) à remplir.
 * Corrige le bug où Gmail encode & en &amp; → « a;prenom ». Désormais, les deux formes
 * sont remplacées.
 */
...
(function () {
    ...
    const DEFAULT_TEMPLATES = {
        "Réponse simple": `<p>Bonjour,</p><p>Merci pour votre message $&nom. Je reviens vers vous rapidement.</p><p>Cordialement,<br>$&signature</p>`,
        "Demande d'informations": `<p>Bonjour $&contact,</p><p>Pourriez‑vous m'envoyer les informations complémentaires concernant&nbsp;…</p><p>Merci d'avance&nbsp;!</p>`,
        Relance: `<p>Bonjour,</p><p>Je me permets de revenir vers vous concernant ma précédente demande du $&date.</p>`,
    ...
    /* -------------------------------------------------- */
    /* Remplacement des variables $&nom / $&amp;nom       */
    /* -------------------------------------------------- */
    function fillTemplateVariables(html) {
        // capture optionnel "amp;" puis nom alphanum/accents/underscore
        const regex = /\$&(amp;)?([\wÀ-ÿ]+)/g;
        const placeholders = [...new Set([...html.matchAll(regex)].map((m) => m[2]))];
        let filled = html;
        for (const key of placeholders) {
            const val = prompt(`Valeur pour « ${key} » :`);
            if (val === null) return null; // annulation
            const raw = new RegExp(`\\$&${key}`, "g");        // $&nom
            const enc = new RegExp(`\\$&amp;${key}`, "g");     // $&amp;nom (HTML encodé)
    ...
    function templateSelect(name) {
        ...
        if (processed === null) return; // annulé
  ...
  #templateSelectionModal .tsm-save{background:#388e3c;margin-right:8px}`;
        document.head.appendChild(Object.assign(document.createElement("style"), { id: "templateModalStyles", textContent: css }));
    }
    ...
    function renderTemplateList() {
        ...
        listMyTemplates().forEach((name) => {
            ...
            const del = Object.assign(document.createElement("span"), { className: "tsm-delete", textContent: "×", title: "Supprimer ce template" });
